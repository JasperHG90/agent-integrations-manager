"""`agent-init doctor` — audit drift across all configured roots.
...
def audit(
    ...
    *,
    ...
    report = DoctorReport()
    resolved_roots = (
        project_roots if project_roots is not None else roots.list_roots()
    )
...
def _audit_project(root: Path, report: DoctorReport) -> None:
    if not root.exists():
        report.findings.append(
            Finding("warning", root, f"project root does not exist: {root}")
        )
        return
    try:
        ...
        report.findings.append(
            Finding("warning", root, "no .agent-init/manifest.json (not initialized)")
        )
        return
    ...
    for managed in m.managed_files:
        ...
        try:
            ...
            report.findings.append(
                Finding("error", root, f"{managed}: malformed markers — {exc}")
            )
            continue
                ...
                if not (skill_dir / _SNAPSHOT_SENTINEL).exists():
                    report.findings.append(
                        Finding(
                            ...
                            "missing .agent-init.complete sentinel",
                        )
                    )
