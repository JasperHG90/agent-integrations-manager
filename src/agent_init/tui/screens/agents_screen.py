...
from agent_init.core import agent_install as install_mod
from agent_init.core import agents, git, manifest, repos
from agent_init.tui.modals.agent_install import AgentInstallConfig, AgentInstallModal
from agent_init.tui.modals.agent_view import AgentViewModal
    ...
    def action_view_current(self) -> None:
        ...
        if qn is None:
            if self.query_one(DataTable).row_count == 0:
                self.app.notify(
                    "no agents indexed — add a repo first", severity="warning"
                )
            else:
