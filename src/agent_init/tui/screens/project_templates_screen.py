...
class ProjectTemplatesScreen(Screen[None]):
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns(
            "name", "template", "profile", "skills", "agents", "mcp", "rules"
        )
        self._populate()
        table.focus()
    ...
    def _on_edit(self, result: TemplateEditResult | None) -> None:
        ...
        new_profile = profile.model_copy(
            update={
                ...
                "skills": [
                    s for s in profile.skills if s.qualified_name in result.skills
                ],
                "agents": [
                    a for a in profile.agents if a.qualified_name in result.agents
                ],
                "mcp_servers": [
                    m for m in profile.mcp_servers if m.alias in result.mcp_servers
                ],
            }
        )
