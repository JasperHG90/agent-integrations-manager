...
`init` seeds default-flagged rules into the project's `.aim/rules/` dir
...
def get(name: str) -> Rule:
    ...
    from aim.core import rule_repos
...
def install_to_project(project_root: Path, name: str) -> RuleInstallResult:
    ...
    """
    get(name)
    from aim.core import declarations as _declarations
    from aim.core import init as _init

    decl = _declarations.load_or_default(project_root)
    if name not in decl.rules:
