        ...
        if key == "args":
            if not isinstance(value, list):
                raise McpOverrideError(
                    f"override 'args' must be a list, got {type(value).__name__}"
                )
            data[key] = [str(v) for v in value if v is not None]
        elif key in ("env", "headers"):
            if not isinstance(value, dict):
                raise McpOverrideError(
                    f"override '{key}' must be a dict, got {type(value).__name__}"
                )
...
def _check_alias_available(
    ...
    if isinstance(servers, dict) and alias in servers:
        ...
        if managed is None and not force:
            raise McpAliasConflictError(
                f"alias {alias!r} already exists in .mcp.json (not managed by aim). "
...
def install(
    ...
    *,
    ...
    if existing is None:
        ...
        existing.overrides = effective_overrides
        result = existing
    manifest.save(project_root, m)
    declarations._update_mcp(project_root, result)
    return result
...
def update(
    ...
    *,
    ...
    if (
        mcp_registry.hash_entry(new_entry) == installed.entry_hash
        and version.definition_hash == installed.current.definition_hash
    ):
        return installed
    ...
    installed.overrides = preserved_overrides
    manifest.save(project_root, m)
...
def delete(project_root: Path, alias: str) -> None:
    ...
    manifest.save(project_root, m)
    declarations._remove_mcp(project_root, alias)
...
def rollback(project_root: Path, alias: str) -> InstalledMcpServer:
    ...
    if target_entry is None:
        raise McpNoHistoryToRollbackError(f"no recorded entry for previous version of {alias}")
