"""Modal: pick the project root to install a plugin target into."""
...
class TargetInstallConfig:
    """Capture the user's choices for installing a plugin target into a project."""
...
class TargetInstallModal(ModalScreen[TargetInstallConfig | None]):
    """Prompt for the project root and ref options when installing a target."""
    ...
    def __init__(self, qualified_name: str, *, initial_project: Path | None = None) -> None:
        """Initialize the modal for the given target.

        Args:
            qualified_name: Fully qualified name of the target to install.
