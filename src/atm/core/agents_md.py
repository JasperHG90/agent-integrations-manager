...
_BEGIN = r"<!-- BEGIN atm: (?P<name>[a-z0-9_-]+) -->"
_END = r"<!-- END atm: (?P=name) -->"
...
def parse(text: str) -> list[Region]:
    ...
    ends = re.findall(r"<!-- END atm: ([a-z0-9_-]+) -->", text)
    if sorted(begins) != sorted(ends):
        raise RegionError(f"unbalanced atm markers: begins={sorted(begins)} ends={sorted(ends)}")
    return regions
...
def merge(existing: str, new_regions: dict[str, str]) -> str:
    ...
    def _replace(match: re.Match[str]) -> str:
        ...
        if not body.endswith("\n"):
            body = body + "\n"
        return f"<!-- BEGIN atm: {name} -->{body}<!-- END atm: {name} -->"
    ...
    if missing:
        ...
        for name, body in missing:
            ...
            if not body.endswith("\n"):
                body = body + "\n"
            suffix_parts.append(f"\n<!-- BEGIN atm: {name} -->{body}<!-- END atm: {name} -->\n")
...
def build(regions: Iterable[tuple[str, str]]) -> str:
    ...
    for name, body in regions:
        ...
        if not body.endswith("\n"):
            body = body + "\n"
        parts.append(f"<!-- BEGIN atm: {name} -->{body}<!-- END atm: {name} -->")
    return "\n".join(parts) + "\n"
