...
def check_logo(text: str) -> tuple[bool, str]:
    ...
    if re.search(r'<img\s+[^>]*src\s*=\s*["\'][^"\']*logo[^"\']*["\']', first_lines):
        return False, "Logo image found near top but not centered. Wrap it in <p align='center'>."
...
def check_section_order(text: str) -> tuple[bool, str]:
    ...
    if len(found_indices) < 3:
        return False, f"Only {len(found_indices)} ladder sections detected; expected at least 3."
